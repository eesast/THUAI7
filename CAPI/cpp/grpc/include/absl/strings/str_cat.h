...
namespace absl
{
    ...
    public:
        ...
        template<typename T, typename = typename std::enable_if<std::is_enum<T>{} && std::is_convertible<T, int>{} && !strings_internal::HasAbslStringify<T>::value>::type>
        ...
        template<typename T, typename std::enable_if<std::is_enum<T>{} && !std::is_convertible<T, int>{} && !strings_internal::HasAbslStringify<T>::value,
                                                     char*>::type = nullptr>
        AlphaNum(T e)  // NOLINT(runtime/explicit)
            :
