...
#define SYS_futex SYS_futex_time64
        ...
        public:
            ...
            static int WaitAbsoluteTimeout(std::atomic<int32_t>* v, int32_t val, const struct timespec* abs_timeout)
            {
                FutexTimespec ts;
                // https://locklessinc.com/articles/futex_cheat_sheet/
                // Unlike FUTEX_WAIT, FUTEX_WAIT_BITSET uses absolute time.
                auto err = syscall(
                    SYS_futex, reinterpret_cast<int32_t*>(v), FUTEX_WAIT_BITSET | FUTEX_PRIVATE_FLAG | FUTEX_CLOCK_REALTIME, val, ToFutexTimespec(abs_timeout, &ts), nullptr, FUTEX_BITSET_MATCH_ANY
                );
                if (err != 0)
                {
                    return -errno;
            ...
            static int WaitRelativeTimeout(std::atomic<int32_t>* v, int32_t val, const struct timespec* rel_timeout)
            {
                FutexTimespec ts;
                // Atomically check that the futex value is still 0, and if it
                // is, sleep until abs_timeout or until woken by FUTEX_WAKE.
                auto err =
                    syscall(SYS_futex, reinterpret_cast<int32_t*>(v), FUTEX_PRIVATE_FLAG, val, ToFutexTimespec(rel_timeout, &ts));
                if (err != 0)
                {
                    return -errno;
                }
            ...
            {
                auto err = syscall(SYS_futex, reinterpret_cast<int32_t*>(v), FUTEX_WAKE | FUTEX_PRIVATE_FLAG, count);
        ...
        private:
            static FutexTimespec* ToFutexTimespec(const struct timespec* userspace_ts, FutexTimespec* futex_ts)
            {
