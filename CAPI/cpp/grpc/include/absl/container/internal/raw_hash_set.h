...
#define ABSL_SWISSTABLE_ENABLE_GENERATIONS
        ...
        public:
            ...
            size_t size() const
            {
                return compressed_tuple_.template get<0>();
            }
            void set_size(size_t s)
            {
                compressed_tuple_.template get<0>() = s;
            }
    ...
    ABSL_OPTION_HARDENED == 1 || !defined(NDEBUG)
        ...
        inline void AssertIsFull(const ctrl_t* ctrl, GenerationType generation, const GenerationType* generation_ptr, const char* operation)
        {
            ...
            if (ctrl == nullptr)
            {
                ABSL_INTERNAL_LOG(FATAL, std::string(operation) + " called on end() iterator.");
            }
            if (ctrl == EmptyGroup())
            {
                ABSL_INTERNAL_LOG(FATAL, std::string(operation) + " called on default-constructed iterator.");
            }
            if (SwisstableGenerationsEnabled())
            {
                if (generation != *generation_ptr)
                {
                    ABSL_INTERNAL_LOG(FATAL, std::string(operation) + " called on invalid iterator. The table could have "
        ...
        inline void AssertIsValidForComparison(const ctrl_t* ctrl, GenerationType generation, const GenerationType* generation_ptr)
        {
            ...
            if (SwisstableGenerationsEnabled())
            {
                if (generation != *generation_ptr)
                {
                    ABSL_INTERNAL_LOG(FATAL, "Invalid iterator comparison. The table could have "
                                             "rehashed since this iterator was initialized.");
                }
        ...
        inline void AssertSameContainer(const ctrl_t* ctrl_a, const ctrl_t* ctrl_b, const void* const& slot_a, const void* const& slot_b, const GenerationType* generation_ptr_a, const GenerationType* generation_ptr_b)
        {
            ...
            if (SwisstableGenerationsEnabled())
            {
                if (generation_ptr_a == generation_ptr_b)
                    return;
                const bool a_is_empty = IsEmptyGeneration(generation_ptr_a);
                const bool b_is_empty = IsEmptyGeneration(generation_ptr_b);
                if (a_is_empty != b_is_empty)
                {
                    ABSL_INTERNAL_LOG(FATAL, "Invalid iterator comparison. Comparing iterator from "
                                             "a non-empty hashtable with an iterator from an empty "
                                             "hashtable.");
                }
                if (a_is_empty && b_is_empty)
                {
                    ABSL_INTERNAL_LOG(FATAL, "Invalid iterator comparison. Comparing iterators from "
                ...
                {
                    ABSL_INTERNAL_LOG(FATAL, "Invalid iterator comparison. Comparing iterator with "
                                             "an end() iterator from a different hashtable.");
                }
                ABSL_INTERNAL_LOG(FATAL, "Invalid iterator comparison. Comparing non-end() "
        ...
        inline void SetCtrl(const CommonFields& common, size_t i, h2_t h, size_t slot_size)
        {
        ...
        void DropDeletesWithoutResize(CommonFields& common, const PolicyFunctions& policy, void* tmp_space);
        ...
        public:
            ...
            ABSL_ATTRIBUTE_NOINLINE raw_hash_set(raw_hash_set&& that) noexcept(
                ...
                settings_(absl::exchange(that.common(), CommonFields{}), that.hash_ref(), that.eq_ref(), that.alloc_ref())
            {
            }
            ...
            ~raw_hash_set()
            {
                ...
                Deallocate<BackingArrayAlignment(alignof(slot_type))>(
                    &alloc_ref(), common().backing_array_start(), AllocSize(cap, sizeof(slot_type), alignof(slot_type))
                );
            ...
            {
                AssertIsFull(position.inner_.ctrl_, position.inner_.generation(), position.inner_.generation_ptr(), "extract()");
                auto node =
            ...
            template<class K = key_type>
            iterator find(const key_arg<K>& key, size_t hash) ABSL_ATTRIBUTE_LIFETIME_BOUND
            {
                auto seq = probe(common(), hash);
            ...
            template<class K = key_type>
            const_iterator find(const key_arg<K>& key, size_t hash) const ABSL_ATTRIBUTE_LIFETIME_BOUND
            {
        ...
        private:
            ...
            struct InsertSlot
            {
                ...
                std::pair<iterator, bool> operator()(const K& key, Args&&...) &&
                {
                    ...
                    if (res.second)
                    {
                        PolicyTraits::transfer(&s.alloc_ref(), s.slot_array() + res.first, &slot);
                    }
                    else if (do_destroy)
                    {
            ...
            ABSL_ATTRIBUTE_NOINLINE void resize(size_t new_capacity)
            {
                ...
                if (old_capacity)
                {
                    ...
                    Deallocate<BackingArrayAlignment(alignof(slot_type))>(
                        &alloc_ref(), old_ctrl - ControlOffset(), AllocSize(old_capacity, sizeof(slot_type), alignof(slot_type))
                    );
                }
        ...
        protected:
            ...
            size_t prepare_insert(size_t hash) ABSL_ATTRIBUTE_NOINLINE
            {
                ...
                auto target = find_first_non_full(common(), hash);
                if (!rehash_for_bug_detection &&
                    ABSL_PREDICT_FALSE(growth_left() == 0 && !IsDeleted(control()[target.offset])))
                {
        ...
        private:
            ...
            {
                auto* h = static_cast<raw_hash_set*>(set);
                PolicyTraits::transfer(&h->alloc_ref(), static_cast<slot_type*>(dst), static_cast<slot_type*>(src));
            }
